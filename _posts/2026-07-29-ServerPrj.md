...
## 레지스트리 서버의 역할
> 레지스트리 서버는 확장 가능한 서버를 구성하기 위한 핵심 역할을 합니다.  
> 레지스트리 서버는 모든 서버들의 목록과 상태를 관리합니다.  
> 서버가 가동될 때 반드시 레지스트리 서버에 먼저 등록을 해야 합니다. 등록하지 않으면 서버가 가동되지 않습니다.  
> 다른 모든 서버들은 레지스트리 서버로부터 서버 목록과 정보를 지속적으로 폴링합니다. 덕분에 서버의 연결이 끊기거나 새로운 게임서버/게이트웨이서버가 추가되었을 때 다른 서버들이 곧바로 알 수 있습니다.

![drawio](/assets/img/posts/2026-07-29/GameServer-레지스트리 서버.drawio.svg){: width="800"}

## 게이트웨이 서버의 역할
1. **게임서버의 브로드캐스팅 네트워크 부하를 감소시킵니다.**  
하나의 던전 내에 100명의 유저가 접속해 있다면, 게임서버가 Send 해야 하는 패킷은 최악의 경우 100배로 증가합니다.  
하지만 게이트웨이 서버가 있으면 게임서버는 게이트웨이 서버에게 **1개의 패킷 + 보낼 대상(유저ID)값 100개** 만 게이트웨이 서버에 Send 하고, 게이트웨이서버가 브로드캐스팅을 대신 해주어 게임서버의 네트워크 부담을 줄입니다.  
때문에 게임서버는 유저가 얼마나 있던 관계없이 안정적으로 로직을 돌릴 수 있습니다.  

1. **클라이언트의 네트워크 세션을 안정적으로 유지하게 해줍니다.**  
클라이언트가 게임서버A에서 게임서버B로 이동하려면 게임서버A의 연결을 끊고 게임서버B에 연결해야 합니다.  
하지만 게이트웨이 서버가 있으면 게이트웨이 서버에서 **클라이언트와 연결된 게임서버 번호만 변경**하면 되기 때문에 클라이언트가 연결을 끊을 필요가 없습니다.  

1. **게임서버를 외부에 공개하지 않습니다.**  
게임서버는 VPC 내의 private subnet에 두어 외부에 공개하지 않고, 게이트웨이 서버만 public subnet에 두어 외부에 공개합니다.  
이런 네트워크 구성이 보안성을 향상시킨다고 볼 수 있지만, 실제로 보안성이 향상되는지는 논란의 여지가 있습니다.  

### 게이트웨이 서버의 단점
1. 게이트웨이 서버를 위한 별도의 머신이 추가되어야 합니다.  
서버 인프라 비용이 증가되고 서버 관리 난이도가 상승됩니다.  

### 게이트웨이 서버 latency 테스트
클라이언트와 게임서버 중간에 게이트웨이서버가 있으므로 latency가 증가할 것으로 예상되어 latency가 얼마나 증가되는지 테스트를 수행합니다. (서버 머신은 모두 AWS EC2 인스턴스 입니다)  
테스트는 아래와 같이 에코 패킷으로 진행하였습니다.  
![drawio](/assets/img/posts/2026-07-29/GameServer-레이턴시테스트.drawio.svg){: width="600"}
<br><br>
- **테스트1 조건:** 동시접속자수 1, 게이트웨이서버 CPU 사용률 1%, 게임서버 CPU 사용률 1%  

| 클라이언트 머신  | 게이트웨이서버 평균 latency | 게임서버 평균 latency | 게이트웨이 서버로 인해 증가된 평균 latency |
| ---------------- | --------------------------- | --------------------- | ------------------------------------------ |
| AWS EC2 인스턴스 | 1.26ms (max 2.57ms)         | 1.81 (max 4.18ms)     | 0.55ms 증가                                |
| 로컬 PC          | 4.90ms (max 15.07ms)        | 5.32ms (max 16.77ms)  | 0.42ms 증가                                |

- **테스트2 조건:** 동시접속자수 1000, 게이트웨이서버 CPU 사용률 20%, 게임서버 CPU 사용률 30%  

| 클라이언트 머신  | 게이트웨이서버 평균 latency | 게임서버 평균 latency | 게이트웨이 서버로 인해 증가된 평균 latency |
| ---------------- | --------------------------- | --------------------- | ------------------------------------------ |
| AWS EC2 인스턴스 | 1.67ms (max 22.35ms)        | 2.06 (max 29.38ms)    | 0.39ms 증가                                |
| 로컬 PC          | 5.40ms (max 11.69ms)        | 6.32ms (max 14.97ms)  | 0.92ms 증가                                |

게이트웨이 서버로 인해 증가되는 latency는 1ms 이내인 것으로 파악되었습니다.  
latency 증가는 거의 없다고 볼 수 있습니다.  
